...
starting_frame = 3
def encodeHandler(secret,filename,seed = 7):
    ...
    new_img = encode(img_pil,secret,seed)
    ...
    vid.set(1,starting_frame)
    ret,frame1 = vid.read()
    frame2 = np.asarray(gray)
    print(PSNR(frame1,frame2))
    img = Image.fromarray(frame1)
    print(decode(img,seed))
    return True
...
def decodeRoute():
    ...
    secret = decodeHandler(filename,seed)
